...
- (void)applicationDidActivate:(NSNotification *)notification {
    ...
    if ([[app localizedName] isEqualToString:@"Google Chrome"]) {
        ...
        AXObserverAddNotification(self.windowObserver, self.chromeElement, (__bridge CFStringRef)NSAccessibilityFocusedUIElementChangedNotification, (void *)self);
    ...
    if (role) {
        if (CFStringCompare(role, kAXTextFieldRole, 0) == kCFCompareEqualTo) {
            ...
            if (value) {
                ...
                if (callback) {
                    callback([self.capturedTitle UTF8String], [self.capturedURL UTF8String]);
                }
            }
            CFRelease(role);
            return;
